...
```python
# 사람과 관계된 속성과 Mnt(2년간 각 제품에 대해 지출한 금액)관련 속성을 제외하고 다 없애주겠다.
df = data.iloc[:,:15]
df.head(20)
```
...
# Income(지출) 컬럼과 각 제품에 대한 지출의 관련도가 높다.
# Kidhome(자녀 수) 컬럼은 제품에 대한 지출 관련도가 확연히 낮다.
```
...
```python
# Education(학력) 컬럼을 post, under로 재구분
...
```python
# Marital_Status(결혼여부) 컬럼을 기혼과 미혼으로 재구분
...
```python
# 자녀컬럼을 새로 생성해 자녀수와 청소년수의 각 컬럼을 합쳐줌
df['Kids'] = df['Kidhome'] + df['Teenhome']
```


```python
# 지출컬럼을 새로 생성해서 각 제품에 대한 지출을 합쳐줌
